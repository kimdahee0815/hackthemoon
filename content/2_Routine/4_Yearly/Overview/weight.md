...
# Weight
```dataviewjs
dv.span("** ⚖️ Weight **") /* optional ⏹️💤⚡⚠🧩↑↓⏳📔💾📁📝🔄📝🔀⌨️🕸️📅🔍✨ */
const calendarData = {
    ...
    defaultEntryIntensity: 50,   // (optional) defaults to 4
    intensityScaleStart: 45,    // (optional) defaults to lowest value passed to entries.intensity
    intensityScaleEnd: 70,     // (optional) defaults to highest value passed to entries.intensity
    entries: [],                // (required) populated in the DataviewJS loop below
}

//DataviewJS loop
for (let page of dv.pages('"content/2_Routine/1_Daily/Daily-Routine"').where(p => p.weight)) {
